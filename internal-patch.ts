...
/**
 * Resolve the live AssistantMessageComponent class.
 *
 * The package's public entry is preferred because Pi's extension loader maps
 * "@mariozechner/pi-coding-agent" to the *running* Pi instance in every mode:
 * jiti aliases for source/dist builds, virtualModules for bundled builds
 * (default since Pi 0.84.3). The legacy path — deriving dist/... file locations
 * from import.meta.resolve — silently resolved to this package's own pinned
 * fallback copy under bundled Pi, patching a dead class while the TUI kept
 * using the class embedded in the bundle.
 */
async function importLiveAssistantMessageComponent(): Promise<unknown> {
	try {
		const publicEntry = (await import("@mariozechner/pi-coding-agent")) as { AssistantMessageComponent?: unknown };
		if (publicEntry && publicEntry.AssistantMessageComponent) {
			return publicEntry.AssistantMessageComponent;
		}
	} catch {
		// Fall through to the legacy deep import below.
	}

	const legacy = await importPiCodingAgentInternal<{ AssistantMessageComponent: unknown }>(
		PI_CODING_AGENT_INTERNAL_MODULES.assistantMessageComponent,
	);
	return legacy.AssistantMessageComponent;
}
...
async function installPatch(themeOverride?: ThinkingThemeLike): Promise<() => void> {
	const rawAssistantMessageComponent = await importLiveAssistantMessageComponent();
	// The theme override (ctx.ui.theme handed over from the extension host) is
	// the live theme object the TUI itself uses. Only fall back to the internal
	// theme module when no override was supplied (tests, non-UI contexts).
	const theme =
		themeOverride ??
		assertThinkingStepsTheme(
			(
				await importPiCodingAgentInternal<{ theme: unknown }>(PI_CODING_AGENT_INTERNAL_MODULES.theme)
			).theme,
		);
...
export async function retainThinkingStepsPatch(themeOverride?: ThinkingThemeLike): Promise<() => Promise<void>> {
	incrementPatchRefCount();
	let cleanup = getPatchCleanup();
	if (!cleanup) {
		const existingInstallPromise = getPatchInstallPromise();
		const installPromise = existingInstallPromise ?? installPatch(themeOverride);
