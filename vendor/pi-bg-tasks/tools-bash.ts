...
import { unwrapSandboxCommand } from "./unwrap.ts";
...
// ── Cooperative renderer hook (LOCAL PATCH, see UPSTREAM.md) ─────────────
// Another extension may inject renderer-only overrides (e.g. pi-config's
// compact-tool-activity, which renders tool rows as one-liners) via
// globalThis[Symbol.for("pi-bg-tasks.renderers")]. Background execution
// above is never affected — only the presentation delegates. Everything is
// resolved lazily at render time, so factory load order between the two
// extensions does not matter.
const EXTERNAL_RENDERERS = Symbol.for("pi-bg-tasks.renderers");
interface ExternalRenderers {
    renderCall?: (args: any, theme: any, context: any) => any;
    renderResult?: (result: any, options: any, theme: any, context: any) => any;
}
function externalRenderers(): ExternalRenderers | undefined {
    return (globalThis as Record<symbol, unknown>)[EXTERNAL_RENDERERS] as
        | ExternalRenderers
        | undefined;
}
...
export function registerBashTool(
    ...
    pi.registerTool({
        ...
        // Lazy renderer delegation — see EXTERNAL_RENDERERS above. When no
        // hook is registered these throw and pi's tool-execution falls back
        // to its native default rendering (try/catch in tool-execution.js),
        // so behaviour without the hook is unchanged.
        get renderShell() {
            return externalRenderers() ? ("self" as const) : ("default" as const);
        },
        renderCall(args: any, theme: any, context: any) {
            const ext = externalRenderers();
            if (ext?.renderCall) return ext.renderCall(args, theme, context);
            throw new Error("pi-bg-tasks: no external renderCall");
        },
        renderResult(result: any, options: any, theme: any, context: any) {
            const ext = externalRenderers();
            if (ext?.renderResult) return ext.renderResult(result, options, theme, context);
            throw new Error("pi-bg-tasks: no external renderResult");
        },
        ...
        async execute(toolCallId, params, signal, onUpdate, ctx) {
            ...
            // The sandbox extension may have wrapped the command (tool_call
            // input mutation). Spawn the wrapped form, but store/display the
            // original so notifications and bg_list stay readable.
            const displayCommand = unwrapSandboxCommand(p.command);
