...
export default function (pi: ExtensionAPI) {
	...
	const renderFooterLines = (
		...
		// LOCAL PATCH (see UPSTREAM.md): surface extension statuses set via
		// ctx.ui.setStatus() — this is where pi-bg-tasks renders its running
		// background-task segments (▶ elapsed · brief command).
		if (footerData) {
			for (const text of footerData.getExtensionStatuses().values()) {
				if (text) parts.push(bracket(theme, text));
			}
		}
		const lines: string[] = [];
		let current = "";
