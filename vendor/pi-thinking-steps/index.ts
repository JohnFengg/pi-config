...
export default function thinkingStepsExtension(pi: ExtensionAPI): void {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			// Hand the live UI theme (ctx.ui.theme) to the patch so it renders with
			// the same theme object the TUI uses; deep-imported theme modules are
			// dead copies under bundled Pi builds.
			registerThinkingPatchRelease(activeScopeKey, await retainThinkingStepsPatch(ctx.hasUI ? ctx.ui.theme : undefined));
